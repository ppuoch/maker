...
class PatientMaker:
    ...
    def on_tick(self) -> None:
        ...
        if natural_spread_bps > 80:
            log.info(
                "[%s] natural spread %.1f bps too wide — skip",
                self.token,
                natural_spread_bps,
            )
            return
    ...
    def _compute_quotes(
        ...
        skew_ticks = int(abs(inv_ratio) * 8)
        ...
        # Long-only: cannot sell more than we hold
        if inv <= 0:
            ask_sz = 0.0
        elif inv < ask_sz:
    ...
    def _force_reduce(self, inv: float, mid: float) -> None:
        ...
        if abs(inv) < 0.1:
            return
        if inv > 0:
            sz = round(min(max(inv * 0.5, 0.5), self.config.base_size), 1)
            if sz < 0.1:
                return
            px = round((self.best_bid or mid) - self.config.tick_size, 2)
            log.warning(
                "[%s] FORCE REDUCE SELL %.1f @ %.2f (local_inv=%.2f)",
                self.token,
                sz,
                px,
                inv,
            ...
            log.warning(
                "[%s] FORCE REDUCE BUY %.1f @ %.2f (local_inv=%.2f)",
                self.token,
                sz,
                px,
                inv,
